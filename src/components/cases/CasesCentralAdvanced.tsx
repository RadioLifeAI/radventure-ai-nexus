...
export function CasesCentralAdvanced() {
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden flex flex-col bg-gradient-to-br from-gray-50 to-blue-50 text-gray-800">
      <HeaderNav />
          ...
          <div className="text-center px-2">
            {filters.specialty && (
              <div className="mb-4 flex flex-col sm:flex-row items-center justify-center gap-2">
                ...
                <div className="bg-blue-50 backdrop-blur-sm rounded-full px-4 py-2 border border-blue-200">
                  <span className="text-blue-700 text-sm">
                    Filtrado por: <strong className="text-blue-800">{filters.specialty}</strong>
                  </span>
                </div>
              </div>
            )}
            
            <h1 className="text-2xl sm:text-3xl lg:text-5xl font-bold bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 bg-clip-text text-transparent mb-2 sm:mb-4">
              Central de Casos Avançada
            </h1>
            <p className="text-gray-600 text-sm sm:text-xl max-w-3xl mx-auto mb-4 sm:mb-6">
              Experiência de aprendizado revolucionária com IA, gamificação e visualizações inteligentes
            </p>
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
              <div className="flex items-center gap-2 bg-white/80 backdrop-blur-sm rounded-full px-3 py-1 sm:px-4 sm:py-2 border border-gray-200 shadow-sm">
                <Sparkles className="h-3 w-3 sm:h-4 sm:w-4 text-yellow-500" />
                <span className="text-xs sm:text-sm text-gray-700">Powered by AI</span>
              </div>
              <div className="flex items-center gap-2 bg-white/80 backdrop-blur-sm rounded-full px-3 py-1 sm:px-4 sm:py-2 border border-gray-200 shadow-sm">
                <Zap className="h-3 w-3 sm:h-4 sm:w-4 text-orange-500" />
                <span className="text-xs sm:text-sm text-gray-700">Real-time Analytics</span>
              </div>
              <div className="flex items-center gap-2 bg-white/80 backdrop-blur-sm rounded-full px-3 py-1 sm:px-4 sm:py-2 border border-gray-200 shadow-sm">
                <Target className="h-3 w-3 sm:h-4 sm:w-4 text-green-500" />
                <span className="text-xs sm:text-sm text-gray-700">Adaptive Learning</span>
              </div>
            </div>
          </div>

          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-4 sm:space-y-6">
            <TabsList className="grid w-full grid-cols-3 bg-white/90 backdrop-blur-sm border border-gray-200 h-auto shadow-sm">
              <TabsTrigger value="advanced-dashboard" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs sm:text-sm p-2 sm:p-3">
                <BarChart3 className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">Dashboard Avançado</span>
                <span className="sm:hidden">Dashboard</span>
              </TabsTrigger>
              <TabsTrigger value="explorer" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs sm:text-sm p-2 sm:p-3">
              ...
              <TabsTrigger value="ai-journey" className="data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 text-xs sm:text-sm p-2 sm:p-3">
