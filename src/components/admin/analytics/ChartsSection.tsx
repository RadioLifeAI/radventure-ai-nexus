...
export function ChartsSection({ caseStats }: ChartsSectionProps) {
  // Valores padrão caso caseStats seja undefined
  const safeCaseStats = {
    totalCases: caseStats?.totalCases || 0,
    activeCases: caseStats?.activeCases || 0,
    completedCases: caseStats?.completedCases || 0
  };

  const data = [
    { 
      name: 'Total', 
      value: safeCaseStats.totalCases, 
      fill: '#3b82f6',
      description: 'Casos totais cadastrados'
    },
    { 
      name: 'Ativos', 
      value: safeCaseStats.activeCases, 
      fill: '#10b981',
      description: 'Casos disponíveis para uso'
    },
    { 
      name: 'Completos', 
      value: safeCaseStats.completedCases, 
      fill: '#8b5cf6',
      description: 'Casos finalizados'
    }
  ];

  const totalCases = safeCaseStats.totalCases;
  const completionRate = totalCases > 0 ? ((safeCaseStats.completedCases / totalCases) * 100).toFixed(1) : '0';
        ...
        {!caseStats ? (
          <div className="flex items-center justify-center h-[350px]">
            <div className="text-center">
              ...
              <p className="text-cyan-600">Carregando dados dos casos...</p>
            </div>
          </div>
        ) : (
          <>
