-- CORREÇÃO DEFINITIVA: Corrigir função submit_daily_challenge
-- Primeiro, garantir que a tabela tem a coluna correta
ALTER TABLE public.user_daily_challenges 
DROP COLUMN IF EXISTS is_correct;

-- Garantir que a coluna was_correct existe
ALTER TABLE public.user_daily_challenges 
ADD COLUMN IF NOT EXISTS was_correct boolean;

-- Recriar função completamente corrigida
...
BEGIN
  ...
  -- Registrar resposta usando was_correct
