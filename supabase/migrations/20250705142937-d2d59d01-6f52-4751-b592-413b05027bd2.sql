-- CORREÇÃO FINAL SIMPLIFICADA - Sem logs complexos

-- 1. Função de validação corrigida
...
BEGIN
  ...
  result := jsonb_build_object(
    ...
    'health_check', jsonb_build_object(
      'cases_system_healthy', events_with_cases >= events_total,
      'prize_system_healthy', events_with_prizes >= events_finished,
      'overall_healthy', (events_with_cases >= events_total AND events_with_prizes >= events_finished)
    )
  );
...
-- 3. View de monitoramento simplificada
...
-- 4. Executar atualização de cache de estatísticas
