-- SISTEMA DE IMAGENS SEPARADO PARA AVATARS E BANNERS DE EVENTOS (CORREÇÃO)
...
-- RLS policies para banners de eventos (apenas novas)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Event banner images are publicly accessible'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'storage' 
    AND tablename = 'objects' 
    AND policyname = 'Admins can manage event banners'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND tablename = 'event_banner_images' 
    AND policyname = 'Everyone can view event banner images'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'event_banner_images' 
    AND policyname = 'Admins can manage event banner images'
  ) THEN
...
DROP TRIGGER IF EXISTS update_event_banner_images_updated_at ON event_banner_images;
