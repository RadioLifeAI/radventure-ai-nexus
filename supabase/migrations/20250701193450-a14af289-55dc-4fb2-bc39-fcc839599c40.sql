...
-- Adicionar novo tipo de transação RadCoin
INSERT INTO radcoin_transactions_log (user_id, tx_type, amount, balance_after, metadata)
SELECT 
  p.id,
  'store_welcome_bonus',
  25,
  p.radcoin_balance + 25,
  '{"reason": "Bônus de boas-vindas da loja", "auto_grant": true}'
FROM profiles p 
WHERE p.type = 'USER' 
  AND p.radcoin_balance >= 0
  AND NOT EXISTS (
    SELECT 1 FROM radcoin_transactions_log rtl 
    WHERE rtl.user_id = p.id AND rtl.tx_type = 'store_welcome_bonus'
  )
LIMIT 5; -- Apenas para alguns usuários como teste

-- Atualizar saldos dos usuários que receberam o bônus
UPDATE profiles 
SET radcoin_balance = radcoin_balance + 25
WHERE id IN (
  SELECT DISTINCT user_id 
  FROM radcoin_transactions_log 
  WHERE tx_type = 'store_welcome_bonus'
);