-- ETAPA 1: CORREÇÃO SQL - Eliminar ambiguidade de variável na função get_case_images_unified
...
DECLARE
  ...
  final_result jsonb;
  array_element jsonb;  -- CORREÇÃO: Renomear para evitar ambiguidade
BEGIN
      ...
      SELECT jsonb_agg(
        CASE 
          WHEN jsonb_typeof(array_element) = 'object' THEN
            ...
            jsonb_build_object(
              'url', COALESCE(array_element->>'url', array_element::text),
              'legend', COALESCE(array_element->>'legend', ''),
              'sequence_order', COALESCE((array_element->>'sequence_order')::integer, 0)
            )
          ELSE
            -- Se é string simples, converter para objeto estruturado
            jsonb_build_object(
              'url', array_element::text,
              'legend', '',
              'sequence_order', 0
            )
        END
      ) INTO final_result
      FROM jsonb_array_elements(image_url_result) AS array_element
      WHERE array_element IS NOT NULL 
      AND (
        (jsonb_typeof(array_element) = 'object' AND array_element ? 'url') OR
        (jsonb_typeof(array_element) = 'string' AND length(array_element::text) > 0)
      );
    ELSE
      final_result := '[]'::jsonb;
    END IF;
